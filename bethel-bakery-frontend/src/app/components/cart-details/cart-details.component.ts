...
//If injecting your own object, you can leave the rest of the code the same.                                      
  constructor() {
    localStorage.setItem("item1",JSON.stringify(this.newCartItemOne) )
     ...
     localStorage.setItem("item2",JSON.stringify(this.newCartItemTwo) ) 
  ...
  */
  ...
  //Key = cartItems to pull from localStorage cart-data-service...
  getCartItem(items): CartItem {
    //this.items = JSON.parse(localStorage.getItem("cartItems")); //This is working to correctly bring whatever varaiable/array/object is pushed into "items" object.
    this.items.push(JSON.parse(localStorage.getItem("item1")));
    this.items.push(JSON.parse(localStorage.getItem("item2")));
    return items; 
